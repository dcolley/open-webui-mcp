<script lang="ts">
	...
	const addUsage = (base_server_id) => {
		const baseServer = $models.find((m) => m.id === base_server_id);

		if (baseServer) {
			if (baseServer.type === 'command') {
				capabilities.usage = baseServer?.meta?.capabilities?.usage ?? false;
			} else {
	...
	onMount(async () => {
		...
		if (server) {
			...
			enableDescription = server?.meta?.description !== null;

			if (server.base_server_id) {
				const base_server = $models
					.filter((m) => !m?.preset && !(m?.arena ?? false))
					.find((m) => [server.base_server_id, `${server.base_server_id}:latest`].includes(m.id));

				console.log('base_server', base_server);

				if (base_server) {
					server.base_server_id = base_server.id;
				} else {
					server.base_server_id = null;
				}
			}

			params = { ...params, ...server?.params };
			...
			filterIds = server?.meta?.filterIds ?? [];
			actionIds = server?.meta?.actionIds ?? [];
			...
			// 	}
			// });
			capabilities = { ...capabilities, ...(server?.meta?.capabilities ?? {}) };

			if ('access_control' in server) {
				accessControl = server.access_control;
			} else {
				accessControl = {};
			}

			console.log(server?.access_control);
			...
			>
				...
				<div class="w-full">
                    ...
                    {$i18n.t('Select a base model')}
									...
									<!-- {#each $servers.filter((s) => (server ? s.id !== server.id : true) && !s?.preset && s?.owned_by !== 'arena') as server}
										<option value={server.id} class=" text-gray-900">{server.name}</option>
									{/each} -->
								</select>
							</div>
						</div>
