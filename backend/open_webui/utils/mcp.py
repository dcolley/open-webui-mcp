...
def apply_extra_params_to_mcp_function(
...
def get_mcp_servers(
    request: Request, server_ids: list[str], user: UserModel, extra_params: dict
) -> dict[str, dict]:
    mcp_servers_dict = {}
    ...
    for server_id in server_ids:
        ...
        for module in server.tools:

          if module is None:
              module, _ = load_tools_module_by_id(server_id)
              request.app.state.MCP_SERVERS[server_id] = module

          extra_params["__id__"] = server_id
          if hasattr(module, "valves") and hasattr(module, "Valves"):
              valves = MCPServers.get_mcp_server_valves_by_id(server_id) or {}
              module.valves = module.Valves(**valves)

          if hasattr(module, "UserValves"):
              extra_params["__user__"]["valves"] = module.UserValves(  # type: ignore
                  **MCPServers.get_user_valves_by_id_and_user_id(server_id, user.id)
              )
          ...
          #     if function_name in mcp_servers_dict:
          #         log.warning(f"Tool {function_name} already exists in another tools!")
          #         log.warning(f"Collision between {server} and {server_id}.")
          #         log.warning(f"Discarding {server}.{function_name}")
          #     else:
          #         mcp_servers_dict[function_name] = tool_dict
